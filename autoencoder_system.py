...
def prepare_data_from_json(json_path, batch_size=32, val_split=0.2):
    ...
    # Ordinal mapping for task_size_category
    ...
    for record in data:
        features = [
            ...
            size_map.get(record.get('task_size_category', 'MEDIUM'), 1)
        ]
        X.append(features)
...
def extract_latent_to_json(model, scaler, input_json, output_json):
    ...
    for r in data:
        X.append([
            r['input_size_mb'], r['cpu_usage_cores_absolute'], r['memory_usage_mb'],
            r['execution_time_normalized'], r['instruction_count'],
            r.get('network_io_mb', 0), r['power_consumption_watts'],
            size_map.get(r.get('task_size_category', 'MEDIUM'), 1)
        ])
...
def main():
    ...
    if not os.path.exists(data_path):
        print(f"Data not found at {data_path}. Skipping training demo.")
        return

    # Train
    train_loader, val_loader, scaler = prepare_data_from_json(data_path)
    model = Autoencoder()
    ...
    # Extract
    extract_latent_to_json(model, scaler, data_path, "dataset/task_profiles_12_features.json")
